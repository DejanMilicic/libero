...
pub fn find_flag_present_test() {
  ...
  let assert Ok("wss://example.com/ws") =
    config.find_flag(args: args, name: "--ws-url")
}

pub fn find_flag_missing_test() {
  let args = ["--ws-url=wss://example.com/ws"]
  let assert Error(Nil) = config.find_flag(args: args, name: "--shared")
}

pub fn find_flag_empty_value_test() {
  let args = ["--ws-url="]
  let assert Ok("") = config.find_flag(args: args, name: "--ws-url")
}
...
pub fn find_flag_empty_args_test() {
  let assert Error(Nil) = config.find_flag(args: [], name: "--ws-url")
}

pub fn find_flag_partial_match_not_found_test() {
  let args = ["--ws-url-extra=foo"]
  let assert Error(Nil) = config.find_flag(args: args, name: "--ws-url")
}
