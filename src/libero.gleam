...
//
//     cd <consumer>/server && gleam run -m libero -- --ws-url=<client_ws_endpoint>
//
...
fn run(config: Config) -> Result(Int, List(GenError)) {
  ...
  case all_errors {
    [] -> {
      ...
      use _ <- result.try(
        write_inputs_manifest(source_files: source_files, config: config)
        |> result.map_error(fn(e) { [e] }),
      )
...
fn merge_dep_module_files(
  ...
  list.fold(files, acc, fn(acc2, file) {
    ...
    use <- bool.guard(when: !string.ends_with(relative, ".gleam"), return: acc2)
  ...
  let seed =
    list.fold(rpcs, set.new(), fn(acc, rpc) {
      dict.fold(rpc.stub_imports, acc, fn(acc2, module_path, type_names) {
        use <- bool.guard(when: is_skipped_module(module_path), return: acc2)
        set.fold(type_names, acc2, fn(acc3, type_name) {
          use <- bool.guard(when: is_primitive_type(type_name), return: acc3)
...
fn write_atoms(
  ...
  // needed regardless of which @rpc functions exist.
  let framework_atoms = [
...
fn build_inject_entry(
  ...
  case func.parameters {
    [glance.FunctionParameter(type_: option.Some(session_t), ..), ..] -> {
      ...
      let return_rendered = render_return_type(func: func, resolver: resolver)
      let inject_fn =
        InjectFn(
          name: func.name,
          module_path: module_path,
...
fn render_return_type(
  ...
  case func.return {
    Some(rt) -> {
      let #(rendered, _) = render_type(t: rt, resolver: resolver)
      rendered
    }
    None -> "Nil"
  }
  ...
  case dict.get(inject_map, label) |> option.from_result {
    ...
    None -> {
      ...
      let errors = case dict.get(inject_types, rendered) |> option.from_result {
        Some(inject_name) ->
          case levenshtein_helper.distance(from: label, to: inject_name) <= 2 {
            True ->
...
fn render_stub_file(rpcs rpcs: List(Rpc), config config: Config) -> String {
  let all_imports =
    rpcs
...
/// lookup).
@external(erlang, \"" <> config.atoms_module <> "\", \"ensure\")
fn ensure_atoms() -> Nil
...
"
      ...
      case simplifile.write(output, content) {
        ...
        Error(cause) -> Ok(CannotWriteFile(path: output, cause: cause))
      }
    })
